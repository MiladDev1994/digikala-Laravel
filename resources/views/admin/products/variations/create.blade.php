...
<form method="post" action="{{route('admin.products.addVar.store' , $product->id)}}" enctype="multipart/form-data" class="p-3">
    @csrf
    @method('post')
...
<div class="w-100 border border-dark mt-4 p-3 {{$product->variety_type == "null" ? 'd-none' : 'd-block'}} position-relative" style="border-radius: 15px">
    @if($product->variety_type == "color")
        @foreach($colors as $color)
            ...
            @endif
        @endforeach
            <a href="{{route('admin.variations.color')}}" target="_blank"  class="position-absolute text-info addVar" style="bottom:  -25px; right: 25px ; font-size: 13px ; text-decoration: none">افزودن رنگ</a>
    @elseif($product->variety_type == "size")
        @foreach($sizes as $size)
            ...
            @endif
        @endforeach
            <a href="{{route('admin.variations.size')}}" target="_blank"  class="position-absolute text-info addVar" style="bottom:  -25px; right: 25px ; font-size: 13px ; text-decoration: none">افزودن سایز</a>
    @elseif($product->variety_type == "Weight")
        @foreach($weights as $weight)
            ...
            @endif
        @endforeach
            <a  href="{{route('admin.variations.weight')}}" target="_blank"  class="position-absolute text-info addVar" style="bottom:  -25px; right: 25px ; font-size: 13px ; text-decoration: none">افزودن وزن</a>
    @elseif($product->variety_type == "volume")
        @foreach($volumes as $volume)
            ...
            @endif
        @endforeach
            <a href="{{route('admin.variations.volume')}}" target="_blank"  class="position-absolute text-info addVar" style="bottom:  -25px; right: 25px ; font-size: 13px ; text-decoration: none">افزودن حجم</a>
    @elseif($product->variety_type == "number")
        @foreach($numbers as $number)
            ...
            @endif
        @endforeach
            <a href="{{route('admin.variations.num')}}" target="_blank"  class="position-absolute text-info addVar" style="bottom:  -25px; right: 25px ; font-size: 13px ; text-decoration: none">افزودن تعداد</a>
    ...
    <div class="row mt-4">
        <div class="col-12 col-md-5 py-2">
            ...
            <a href="{{route('admin.warranties')}}" target="_blank" class="text-info addVar d-block px-4 mt-1" style="font-size: 13px ; text-decoration: none">افزودن گارانتی</a>
        ...
        <div class="col-4 col-md-2 py-2 d-flex align-items-center justify-content-center">
            <p class="mt-1 me-2 opacity-50">پرفروش</p>
            <input name="moreSell" type="checkbox" class="form-check-input bg-dark h3 " style="border: none ; outline: none">
        </div>
        <div class="col-4 col-md-2 py-2 d-flex align-items-center justify-content-center">
            <p class="mt-2 me-2 opacity-50">شگفت‌انگیز</p>
            <input name="spesial" type="checkbox" class="form-check-input bg-dark h3 " style="border: none ; outline: none">
        </div>
    </div>
...
@if($errors->all())
    @foreach($errors->all() as $error)
        <div class="alert alert-danger d-inline-block p-0 mt-5" role="alert">{{$error}}</div>
    @endforeach
@endif
