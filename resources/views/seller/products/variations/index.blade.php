...
<style>
    ...
    <div class="row mt-4 px-4">
        ...
        <div class="col-6 col-md-1 d-flex justify-content-center p-0 position-relative  py-3 adminFilterBox opacity-0">
    ...
    <div class="w-100 bg-dark" style="height: 50px;margin-top: 5px;display: flex;align-items: center;justify-content: space-between;padding: 0 10px;">
        <div class="m-auto d-flex align-items-center justify-content-between" style="width: 97%">
            ...
            <div class=" text-center" style="font-size: 15px ; font-weight: 1000 ; width: 330px"> نام محصول</div>
            <div class=" text-center" style="font-size: 15px ; font-weight: 1000 ; width: 150px">دسته</div>
            <div class=" text-center" style="font-size: 15px ; font-weight: 1000; width: 90px"> دسترسی </div>
        </div>
    ...
    function addFilter(s,c,t,a,n,d,o){
        ...
        for (i = 0 ; i < varJson.length ; i++){
            ...
            addElementNameToDiv.innerHTML = '<div class="w-100  py-2 overflow-hidden opacity-75" style="height: 70% ; overflow=hidden ; font-size: 12px"> ' +
                varJson[i].name + ' | ' + varJson[i].var_name + ' | ' + 'گارانتی' + ' ' + varJson[i].warranty_name + ' ' + varJson[i].warranty_time
                + '</div>' +
            ...
            addElementCategoryToDiv.innerText = varJson[i].category_name;
            addProductBoxmainTop.appendChild(addElementCategoryToDiv);
            ...
            addElementAccessToDiv.innerHTML ='' +
                '<div class="position-relative togleBoxOne opacity-50" style="width: 40px ; height: 25px ; background-color: #01c394 ; border-radius: 15px ; cursor: pointer ">' +
            ...
            box.childNodes[i].childNodes[0].childNodes[4].childNodes[0].classList.add('togleBox' + i);
            box.childNodes[i].childNodes[0].childNodes[4].childNodes[0].childNodes[0].classList.add('togle' + i);
            box.childNodes[i].childNodes[0].childNodes[4].childNodes[0].childNodes[3].classList.add('deactive' + i);
            box.childNodes[i].childNodes[0].childNodes[4].childNodes[0].childNodes[2].classList.add('active' + i);

            let togleBox = box.childNodes[i].childNodes[0].childNodes[4].childNodes[0];
            let togle = box.childNodes[i].childNodes[0].childNodes[4].childNodes[0].childNodes[0];
            let deactive = box.childNodes[i].childNodes[0].childNodes[4].childNodes[0].childNodes[3];
            let active = box.childNodes[i].childNodes[0].childNodes[4].childNodes[0].childNodes[2];
            ...
            if (varJson[i].auth_id === varJson[i].user_id){
                ...
                deactiveB.addEventListener('click' , function (e){
                    ...
                    $.ajax({
                        url:'{{route('seller.varieties.active')}}',
                        method:'POST',
                        data:{
                            variety : varietytId,
                            active : 0,
                ...
                sentButton.addEventListener('click' , function (e){
                    ...
                    if (priceBox.value == 0){
                        ...
                        $.ajax({
                            url:'{{route('seller.varieties.values')}}',
                            method:'POST',
                            data:{
                                variety : varietytId,
